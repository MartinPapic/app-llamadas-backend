...
class MigrationConfig(private val jdbcTemplate: JdbcTemplate) {
    ...
    fun runMigration() = CommandLineRunner {
        ...
        try {
            // 1. Convertir CONTESTA -> CONTACTADO_EFECTIVO
            val contestadas = jdbcTemplate.update(
                "UPDATE llamada SET resultado = 'CONTACTADO_EFECTIVO' WHERE resultado = 'CONTESTA'"
            )
            if (contestadas > 0) logger.info("Migrados $contestadas registros de CONTESTA a CONTACTADO_EFECTIVO")

            // 2. Convertir NO_CONTESTA / OCUPADO / INVALIDO -> NO_CONTACTADO
            val noContestadas = jdbcTemplate.update(
                "UPDATE llamada SET resultado = 'NO_CONTACTADO' WHERE resultado IN ('NO_CONTESTA', 'OCUPADO', 'INVALIDO', 'NUMERO_INVALIDO')"
            )
            if (noContestadas > 0) logger.info("Migrados $noContestadas registros de estados fallidos a NO_CONTACTADO")
